...
export default async function HomePage() {
  ...
  return (
    <PhoneFrame>
      <section className="relative overflow-hidden rounded-[34px] bg-gradient-to-br from-[#7bc8ff] via-[#3a98ff] to-[#1766e6] p-5 text-white shadow-[0_22px_50px_rgba(37,99,235,0.26)]">
        <div className="pointer-events-none absolute -right-12 -top-10 h-40 w-40 rounded-full bg-white/24 blur-2xl" />
        <div className="pointer-events-none absolute bottom-0 right-2 text-7xl opacity-20">☁️</div>
        <div className="pointer-events-none absolute left-5 top-6 text-xl opacity-70">⭐</div>

        <header className="relative z-10 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="flex h-16 w-16 items-center justify-center rounded-full border-4 border-white/60 bg-white text-4xl shadow-sm">👦</div>
            <div>
              <p className="rounded-full bg-white/18 px-3 py-1 text-xs font-black text-white/90">星星練習王</p>
              <h1 className="mt-2 text-[28px] font-black leading-tight tracking-[-0.04em]">Hi，星見！</h1>
            </div>
          </div>
          <Link href="/parent/login" className="flex h-11 w-11 items-center justify-center rounded-full bg-white/22 text-xl backdrop-blur active:scale-95">
            ⚙️
          </Link>
        </header>

        <div className="relative z-10 mt-5 rounded-[30px] bg-white/94 p-4 text-[#172033] shadow-[0_14px_28px_rgba(30,64,175,0.12)]">
          <div className="flex items-start gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-[20px] bg-[#e9f4ff] text-2xl">📋</div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-lg font-black">今日任務</h2>
                <span className="rounded-full bg-[#fff0a8] px-3 py-1 text-sm font-black text-[#1d4ed8]">1 / 4</span>
              </div>
          ...
          <Link href="/practice" className="mt-4 flex h-12 items-center justify-center rounded-[22px] bg-gradient-to-r from-[#2f8cff] to-[#1766e6] text-base font-black text-white shadow-[0_12px_22px_rgba(37,99,235,0.24)] active:scale-[0.99]">
            開始練習
          </Link>
        </div>
      </section>
          ...
          {collections.map((collection) => {
            ...
            return (
              <Link key={collection.id} href="/collection" className="block rounded-[24px] bg-[#f8fbff] p-3 shadow-[0_8px_18px_rgba(30,64,175,0.06)] active:scale-[0.99]">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-[20px] bg-white text-2xl shadow-sm">
                      <CollectionIcon name={collection.name} />
                    </div>
                    <div>
                      <div className="text-base font-black text-[#172033]">{collection.name}</div>
                      <div className="text-xs font-bold text-[#7b8aa3]">還差 {Math.max(collection.total - collection.owned, 0)} 張</div>
                    </div>
                  </div>
                  <div className="text-base font-black text-[#2f8cff]">{collection.owned}/{collection.total}</div>
                </div>
      ...
      <div className="rounded-[28px] bg-white/70 p-4 text-center text-sm font-bold text-[#5f6f89] shadow-sm">
